...
type Props = {
  pickedHook: [VizTypeFilter, Dispatch<SetStateAction<VizTypeFilter>>]
}

export default function SelectMouseClicksType({ pickedHook }: Props) {
  const vizTypes: VizTypeFilter[] = [
  ...
  return (
    <Listbox value={picked} onChange={setPicked}>
      <div className="relative z-50 min-w-[10rem]">
        <Listbox.Button
          as="button"
          className="inline-flex w-full items-center justify-center gap-x-1 rounded border border-primary bg-primary/50 py-2 pl-3 pr-2 text-center text-sm font-medium tracking-tight text-white transition hover:opacity-80 disabled:cursor-not-allowed disabled:opacity-50 dark:border-secondary dark:bg-secondary/50 lg:px-2 lg:py-1.5"
        >
        ...
        <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
          <Listbox.Options className="absolute mt-2 max-h-60 w-full overflow-auto rounded border border-gray-300 bg-gray-100 py-2 shadow lg:w-full">
            {vizTypes.map((viz: VizTypeFilter, vizIdx: number) => (
              <Listbox.Option
                key={`viz-${vizIdx}`}
                className={({ active }) =>
